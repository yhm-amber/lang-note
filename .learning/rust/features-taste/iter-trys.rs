...
fn main() 
{
	...
	// define unfold
	fn iterate<T, F>(init: T, mut f: F) -> impl Iterador<Item = T>
	where
		T: std::clone::Clone,
	...
	fn unfold<T, S, F>(init: S, mut f: F) -> impl Iterador<Item = T>
	where
		F: FnMut(S) -> Option<(T, S)> + std::clone::Clone,
		S: Clone, T: std::clone::Clone
	{
		std::iter::successors(
				f(init.clone()), 
				move |&(_, ref s)| f(s.clone()))
			.map(|(value, _)| value)
	}
	
	{
		let fib = unfold((0, 1), |(a, b)| Some((a, (b, a + b))));
		if let Some((head, tail)) = fib.pair()
		{
