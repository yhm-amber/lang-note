...
c (cbind(c(1,2), c(3,4), c(5,6))) ; # [1] 1 2 3 4 5 6
c (rbind(c(1,2), c(3,4), c(5,6))) ; # [1] 1 3 5 2 4 6

### 👺 转换回向量的时候也是先数完一列再数下一列。
...
sum (cbind(c(1,2), c(3,4), c(5,6)) * cbind(c(1,2), c(3,4), c(5,6))) ; # [1] 91

### 👺 矩阵内积 👹

crossprod (cbind(c(1,2), c(3,4), c(5,6)), cbind(c(1,2), c(3,4), c(5,6))) ;
(function () { `%x%` <- crossprod ; cbind(c(1,2), c(3,4), c(5,6)) %x% cbind(c(1,2), c(3,4), c(5,6)) }) () ;
...
### 👺 左边向量中每个元素规定结果中每行特征，故左向量长度等于结果行数；右边向量中每个元素规定结果中每列特征，故右向量长度等于结果列数。
...
### 👺 可见，在这里，向量仍然是被视为与单列矩阵（而不是单行矩阵）等价。
...
apply (c(1,2,3) %o% c(1, -1), 1, sum) ; # [1] 0 0 0
apply (c(1,2,3) %o% c(1, -1), 2, sum) ; # [1]  6 -6

### 👺 模式 1 是横着聚合每一行的所有列。
### 👺 模式 2 是竖着聚合每一列的所有行。

# apply (c(1,2,3) %o% c(1, -1), 3, sum) ;
# apply (c(1,2,3) %o% c(1, -1), 0, sum) ;
### 👹 没有别的模式序号。
