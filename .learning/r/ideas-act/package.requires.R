...
function (requires)
{
	.requires = base::data.frame(package = base::unique(requires))
	...
	.requires$local_at = .requires$package |> vapply(\ (P) base::system.file(package = P), FUN.VALUE = "character")
	.requires$local_ver = seq(nrow(.requires)) |> vapply(ver_local (.requires, 'local_at', 'package'), FUN.VALUE = "character")
	.requires$repo_at = seq(nrow(.requires)) |> vapply(ver_repos (.requires, 'Repository', 'package'), FUN.VALUE = "character")
	.requires$repo_ver = seq(nrow(.requires)) |> vapply(ver_repos (.requires, 'Version', 'package'), FUN.VALUE = "character")
...
utils::install.packages(full[full$required, 'package'])
